...
for i in range(max(len(op_1),len(op_0))):
    ...
    print('第%d次cache_0对'%(i+1),'第%d组操作后状态'%index_0)
    cache_0[index_0][0].print_cache_line()
    print('第%d次cache_1对'%(i+1),'第%d组操作后状态'%index_1)
    cache_1[index_1][0].print_cache_line()
    print('####################################################')

cache_set_title = PT(['set num', 'cache line num', 'tag', 'state','data'])
for i in range(cache_line_per_set):
    cache_set_title.add_row([index_0,i,cache_0[index_0][i].tag,cache_0[index_0][i].state,cache_0[index_0][i].data])
