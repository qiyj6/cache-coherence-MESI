...
def apply_cache_line(cache_set,tag):
    if(set_full==True): #如果满，则随机替换
        ...
        cache_set[set_full(cache_set)].tag=tag
        cache_set[set_full(cache_set)].state=EXCLUSIVE
        cache_set[set_full(cache_set)].data='wait'
    else:
        cache_set[set_full(cache_set)].tag=tag
        cache_set[set_full(cache_set)].state=EXCLUSIVE
        cache_set[set_full(cache_set)].data='wait'
...
def cache_op_hit(cache_op_d,cache_op_id,index,tag,offset_in_set_d,op):
    if(op==1):
        if(cache_op_d[index][offset_in_set_d].state==EXCLUSIVE):
            ...
            if(offset_in_set_id>=0):
                cache_op_id[index][offset_in_set_id].state=INVALID


def cache_op_miss(cache_op_d,cache_op_id,index,tag,op):
    if(op==0):
        if(cache_HorM(cache_op_id,index,tag)==False): #其他cache line也不命中
            #申请一个新的cache line 装入内存的数据
            cache_op_d[index]=apply_cache_line(cache_op_d[index],tag)
