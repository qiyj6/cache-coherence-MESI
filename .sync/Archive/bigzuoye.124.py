...
class cache_line:
    ...
    def print_cache_line(self):
        print("tag=0x%05x"%self.tag, "state=", self.state)
...
def cache_set_print(cache_num,index):
    cache_set_title = PT(['cache num','set num', 'cache line num', 'tag', 'state','data'])
    for i in range(cache_line_per_set):
        cache_set_title.add_row(['cache_num',index_0,i,cache_num[index_0][i].tag,cache_num[index_0][i].state,cache_num[index_0][i].data])
    return 
    print(cache_set_title)
...
for i in range(max(len(op_1),len(op_0))):
    ...
    print('第%d次cache_0对'%(i+1),'第%d组操作前状态'%index_0)
    cache_0[index_0][0].print_cache_line()
    print('第%d次cache_1对'%(i+1),'第%d组操作前状态'%index_1)
    cache_1[index_1][0].print_cache_line()
    print('\n')
    if(cache_0_hit!=-1):
    ...
    cache_set_print(cache_0,index_0)
    print('####################################################')
