...
class bus: #地址信号，数据信号，数据有效信号,状态信号(总线是否被占用)，读写广播信号，读写广播响应
    ...
    def available(self):
        ...
        self.device_id=-1
        self.data_valid=0
    def occuied(self,tag,index,broadcast,device_id=-1,data_valid=0,data=[]):
        self.tag = tag
        self.index = index
        self.data=data
        self.state=1
...
def read_from_bus(cache_op_d,cache_op_id,mem_list,tag,index,offset_in_set_d,bus=bus()):
    ...
    print('cache____=',cache_op_d[0][1])
    j=0
...
def cache_op_miss(cache_op_d,cache_op_id,mem_list,index,tag,op,offset_num,data_i,bus=bus()):
    ...
    cache_op_d,cache_op_id,mem_list=read_from_bus(cache_op_d,cache_op_d,cache_op_id,mem_list,tag,index,offset_in_set)
