...
class bus: #地址信号，数据信号，数据有效信号,状态信号(总线是否被占用)，读写广播信号，读写广播响应
    ...
    def available(self):
        ...
        self.device_id=-1
        self.data_valid=0
    def occuied(self,tag,index,broadcast,device_id=-1,data_valid=0,data=-1):
        self.tag = tag
        self.index = index
        self.data=data
        self.state=1
...
def cache_op_hit(cache_op_d,cache_op_id,mem_list,index,tag,offset_in_set_d,op,offset_num,data_i,bus=bus()):
    ...
    if(op==1):  
        if(cache_op_d[index][offset_in_set_d].state==(EXCLUSIVE or MODIFIED)):
            ...
            bus.occuied(tag,index,offset_num,data=cache_op_d[index][offset_in_set_d].data,broadcast=op,device_id=all_device,data_valid=1) 

    cache_op_id,mem_list=state_I_mem_fresh(cache_op_id,mem_list,bus)

    return cache_op_d,cache_op_id,mem_list

def read_from_bus(cache_op_d,cache_op_id,mem_list,tag,index,offset_in_set_d,bus=bus()):
    bus.occuied(tag=tag,index=index,broadcast=0,device_id=the_other_cache_id)
    if(bus.broadcast==0 and bus.device_id==the_other_cache_id):
        id_set_offset,id_offset_state=cache_HorM(cache_op_id[index],tag)
        bus.response=id_set_offset
        if(id_set_offset!=-1):
            bus.state_response=id_offset_state
            if(id_offset_state==MODIFIED):
                bus.data_valid=1
                bus.data=cache_op_id[index][id_set_offset].data
                cache_op_id[index][id_offset_state].state=SHARED
            else: #cache_id 处于E或S
                cache_op_id[index][id_offset_state].state=SHARED
    else:
        pass

    if(bus.response==-1 or bus.state_response==(SHARED or EXCLUSIVE)):
        bus.data=mem_list[tag][index]
        bus.data_valid=1
    elif(bus.state_response==MODIFIED):
        mem_list[tag][index]=bus.data
    else:
        pass
    
    print('index=',index)
    print('offset=',offset_in_set_d)
    j=0
    for i in cache_op_d[0][0].data:
        print(j)
        j=j+1
    cache_op_d[index][offset_in_set_d].data=bus.data
...
def cache_op_miss(cache_op_d,cache_op_id,mem_list,index,tag,op,offset_num,data_i,bus=bus()):
    data=bin(data_i)[2:].rjust(8,'0')
    #进入该函数的前提是cache_d不命中
    cache_op_d,mem_list,offset_in_set=apply_line_in_set(cache_set=cache_op_d[index],mem_list=mem_list,tag=tag,index=index)
    ...
    if(op==1): #写不命中
        ...
        if(cache_op_d[index][offset_in_set].state==EXCLUSIVE):
            ...
            cache_op_d[index][offset_in_set].data[offset_num]=data
            bus.occuied(tag,index,offset_num,data=cache_op_d[index][offset_in_set].data,broadcast=op,data_valid=1,device_id=all_device)
    else: 
        pass
...
print('cache data=',cache_0[0][0].data[63])
...
for i in range(max(len(op_1),len(op_0))):
    ...
    if(op_1[i]==1 and op_0[i]==0 and op_address_0[i]==op_address_1[i]):
        ...
        cache_0_hit,none=cache_HorM(cache_num=cache_0,index=index_0,tag=tag_0)
        ...
        cache_1_hit,none=cache_HorM(cache_num=cache_1,index=index_1,tag=tag_1)
