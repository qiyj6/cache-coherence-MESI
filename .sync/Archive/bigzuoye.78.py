...
def cache_op_miss(cache_op_d,cache_op_id,mem_list,index,tag,op,offset_num,data_i,cache_op_id_hit,bus=bus()):
    data=bin(data_i)[2:].rjust(8,'0')
    #进入该函数的前提是cache_d不命中。需要cache_id判断自己是否命中
    bus.occuied(tag,index,offset_num,data,broadcast=-1,) #总线被cache_d占用,暂时不广播读还是写
    cache_op_id_hit=cache_HorM(cache_op_id,bus.index,bus.tag) #cache_id从总线上拿到地址信息，判断是否命中相关地址的数据
    bus.response=cache_op_id[index][cache_op_id_hit].state
    if(op==0):
        bus.broadcast=op
        if(bus.response==INVALID): #其他cache line也不命中
            ...
            if(bus.response==EXCLUSIVE or SHARED):   #其他cache 是E，将其他cache的数据装入当前cache，cache状态都变为S
                cache_op_d[index],mem_list[tag][index],offset_in_set=apply_cache_line(cache_op_d[index],mem_list[tag][index],tag)
                bus.device_id=0
    ...
    if(bus.broadcast==0 and bus.device_id==0):
        bus.data=cache_op_id[bus.index][cache_op_id_hit].data
        cache_op_id[bus.index][cache_op_id_hit].state=SHARED
        bus.data_valid=1
...
for i in range(max(len(op_1),len(op_0))):
    ...
    if(bus.device_id==1 and bus.broadcast==0):
        bus.response=cache_HorM(cache_num=cache_1,index=index_0,tag=tag_0)

    if(bus.device_id==0 and bus.broadcast==0):
        bus.response=cache_HorM(cache_num=cache_0,index=index_1,tag=tag_1)
    
    if(cache_1_hit==-1):
        cache_1,cache_0,mem_list=cache_op_miss(cache_1,cache_0,mem_list,index_1,tag_1,op_1[i],offset_1,i+1,bus.response)
    if(cache_0_hit==-1):
        cache_0,cache_1,mem_list=cache_op_miss(cache_0,cache_1,mem_list,index_0,tag_0,op_0[i],offset_0,i+1,bus.response)
