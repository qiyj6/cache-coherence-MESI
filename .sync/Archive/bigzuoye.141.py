...
def set_avail(cache_set,mem_list,index,bus=bus()):
    set_avail_offset=-1
    for i in range(cache_line_per_set):
        if(cache_set[i].state==INVALID):
            set_avail_offset=i
            break    #cache组不满，返回不满位置的offset_in_set
        else:
            continue
    if(set_avail_offset==-1): #set full
        num=random.randint(0,cache_line_per_set-1)
        if(cache_set[num].state==MODIFIED):
            bus.occuied(tag=cache_set[num].tag,index=index,data=cache_set[num].data,broadcast=1,device_id=mem_id)
    return cache_set,mem_list
...
def cache_op_miss(cache_op_d,cache_op_id,mem_list,index,tag,op,offset_num,data_i,state_response,response,bus=bus()):
    data=bin(data_i)[2:].rjust(8,'0')
    #进入该函数的前提是cache_d不命中
    bus.occuied(tag,index,offset_num,data,broadcast=op) #总线被cache_d占用,暂时不广播读还是写
    cache_op_d,mem_list=set_avail(cache_op_d,mem_list)
    if(op==0):
        if(response==-1): #其他cache line也不命中
            ...
            elif(state_response==MODIFIED):   #其他cache 是M，将其他cache的数据装入当前cache，写入主存，cache状态都变为S
                cache_op_d[index],mem_list[tag][index],offset_in_set=apply_cache_line(cache_op_d[index],mem_list[tag][index],tag)
                bus.device_id=3
                cache_op_d[index][offset_in_set].state=SHARED   
    else: #写不命中
        ...
        if(response==-1): #其他cache不命中
            ...
            if(state_response==EXCLUSIVE or SHARED):
                cache_op_d[index],mem_list[tag][index],offset_in_set=apply_cache_line(cache_op_d[index],mem_list[tag][index],tag)
                cache_op_d[index][offset_in_set].data[offset_num]=data
                bus.device_id=3
                bus.data=data
    ...
    if(bus.broadcast==0 and bus.device_id==3):
        bus.data=cache_op_id[index][response].data
        mem_list[tag][index]=bus.data
        cache_op_id[bus.index][bus.response].state=SHARED
        bus.data_valid=1

    if(bus.broadcast==1 and bus.device_id==2):
        mem_list[tag][index]=bus.data

    if(bus.broadcast==1 and bus.device_id==3):
        mem_list[tag][index]=bus.data
        cache_op_id[bus.index][response].state=INVALID

    if(bus.broadcast==1 and bus.device_id==4):
        bus.data=cache_op_id[index][response].data
        mem_list[tag][index]=bus.data
        bus.data=bus.data_buffer
        cache_op_id[bus.index][response].state=INVALID
