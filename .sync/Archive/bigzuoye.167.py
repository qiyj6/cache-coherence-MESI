...
class cache_line:
    def __init__(self, state=INVALID,tag=0x00000,data=[]):
        self.state = state
        self.tag = tag
        self.data = data
...
def cache_set_print(cache_num,index):
    ...
    for i in range(cache_line_per_set):
        cache_set_title.add_row([index,i,cache_num[index][i].tag,cache_num[index][i].state,cache_num[index][i].data[63]])
...
def cache_set_init():
    ...
    for i in range(set_num):
        ...
        for j in range(cache_line_per_set):
            ...
            for k in range(cache_line_databyte):
                cache_set_list[i][j].data.append('0')
    print('data list length=',len(cache_set_list[32][3].data))
...
for i in range(max(len(op_1),len(op_0))):
    ...
    print(cache_set_print(cache_0,index_0))
    print('第%d次cache_1操作前cache set如下'%(i+1))
    print(cache_set_print(cache_1,index_1))
    if(cache_0_hit!=-1):
