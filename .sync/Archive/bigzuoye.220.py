...
def mem_write(source,mem_data,tag,index):
    # mem_addr_high=tag*4096+index*64
    for i in range(cache_line_databyte):
        mem_data[i]=source[i]
    return mem_data

def mem_print(addr,mem_data):
    cache_set_title = PT(['mem address', 'data'])
    for i in range(cache_line_per_set):
        cache_set_title.add_row([addr,mem_data[addr]])
    return cache_set_title
...
def apply_cache_line(cache_set,mem_list,tag,index):

    num=set_avail(cache_set)
    if(num==-1): #满,则随机替换
        num=random.randint(0,3)
        if(cache_set[num].state==MODIFIED):
            mem_list=mem_write(cache_set[num].data,mem_list,tag,index)
        cache_set[num].tag=tag
        for i in range(cache_line_databyte):
            cache_set[num].data[i]=mem_list
        # cache_set[num].data='wait load from other place'
    else:
        cache_set[num].tag=tag
        # cache_set[num].data='wait load from other place'
    return cache_set,mem_list,num
...
def cache_op_hit(cache_op_d,cache_op_id,mem_list,index,tag,offset_in_set_d,op,offset_num):
    if(op==1):
        if(cache_op_d[index][offset_in_set_d].state==EXCLUSIVE):
            ...
            cache_op_d[index][offset_in_set_d].data[offset_num]='10011001'
            mem_list=mem_write(cache_op_d[index][offset_in_set_d].data,mem_list,tag,index)
